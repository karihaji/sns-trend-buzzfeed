...
ただし、抽出しただけの語は「急上昇」扱いしません。`evidenceCount` を履歴に残し、次回取得時に `evidenceChange` を比較します。伸びが確認できた語だけを「伸びている観測ワード」に出し、初回または比較不能な語は「観測候補（伸び確認待ち）」に分けます。記事本文やニュース一覧は表示しません。

画面では、Google Trends RSSに載った公開トレンドを「いま話題の実トレンド」、伸びが確認できた独自観測語を「伸びている観測ワード」、横ばいでも継続して出ている構文・あるある・企画型を「よく使われる投稿ネタ」として分けて表示します。
